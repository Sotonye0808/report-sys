/**
 * app/api/report-update-requests/[id]/approve/route.ts
 * POST /api/report-update-requests/:id/approve  — APPROVE a pending update request
 */
...
export async function POST(
    ...
    try {
        ...
        const updated = await db.reportUpdateRequest.update({
            where: { id },
            data: { status: ReportUpdateRequestStatus.APPROVED, reviewedById: auth.user.id, updatedAt: new Date() },
        });
        ...
        if (requester) {
            await createNotification({
                userId: requester.id,
                type: NotificationType.REPORT_UPDATE_APPROVED,
                title: "Report Update Approved",
                message: "Your report update request has been approved.",
