...
| `lib/data/orgUnit.ts` + `orgUnitMatcher.ts` | Polymorphic OrgUnit CRUD + tree helpers + multi-root + `unitInScope` matcher with graph cache | `lib/data/orgUnit.ts`, `lib/data/orgUnitMatcher.ts` | `prisma/*` |
| `lib/data/role.ts`  | Runtime Role CRUD (system + custom) with SUPERADMIN immutability + capability subset enforcement + scope-pinning + `resolveRoleByCode` fallback | `lib/data/role.ts` | `prisma/*`, `config/roles.ts` |
| `lib/data/publicCopy.ts` | Sanitised + deep-merged read wrapper over `loadAdminConfig` for public-page namespaces | `lib/data/publicCopy.ts` | `lib/data/adminConfig.ts` |
| `lib/data/reconcile.ts` | Idempotent reconciliation engine: mirrors legacy Campus + OrgGroup into OrgUnit, seeds Role rows from ROLE_CONFIG, back-fills `unitId`/`roleId` columns. Used by the seed script and the admin Reconcile action | `lib/data/reconcile.ts` | `prisma/*`, `config/roles.ts` |
| `lib/data/importPipeline.ts` (extended) | Now hosts `parseXlsx` (SheetJS lazy-import) + `parseSpreadsheet` dispatcher + `SpreadsheetParseError` | same | `xlsx` |
| `modules/admin-config/components/OrgUnitTreeEditor.tsx` | Multi-root tree editor with per-node add/edit/promote/archive | same | `app/api/org/units/*` |
| `modules/admin-config/components/RolesEditorV2.tsx` | Runtime CRUD for the Role table with CREATE flow + scope-pin + capability subset | same | `app/api/roles/*` |
| `modules/admin-config/components/PublicCopyEditors.tsx` | LandingCopyEditor + HowItWorksEditor + SimplePageEditor (about/privacy/terms) + ReconcilePanel | same | `app/api/admin-config/[ns]`, `/api/admin-config/reconcile` |
| `app/api/org/units/*` | OrgUnit REST surface (list/tree/CRUD/promote/archive) | same | `lib/data/orgUnit.ts` |
| `app/api/roles/*`     | Role REST surface (list/CRUD/archive) | same | `lib/data/role.ts` |
| `app/api/public-copy/[ns]` | Public-read endpoint for sanitised + merged public-page payloads | same | `lib/data/publicCopy.ts` |
| `app/api/admin-config/reconcile` | SUPERADMIN-only POST that runs `reconcileSubstrate({ dryRun })` | same | `lib/data/reconcile.ts` |
...
49) FormAssignmentRule editor: `/templates/[id]?tab=assignments` mounts `TemplateAssignmentsEditor` which CRUDs rules via `/api/form-assignment-rules`. Scope is expressed as `campusIds: string[]` and `orgGroupIds: string[]`; **empty array = applies to all campuses/groups** (templates are platform-scoped, so a campus-bound role like USHER picks up its own campus from the user profile at materialise time). Legacy single-value `campusId`/`orgGroupId` columns stay readable for back-compat; new writes always go through the array columns. `ruleMatchesUser(rule, user)` (in `lib/data/formAssignmentRule.ts`) is the canonical match helper used by `recurringAssignmentService.materialiseAssignmentsForUser`. Server validates metric-subset belongs to the template; no campus/group requirement. Closes the prior gap where rules could only be inserted via DB writes.
...
55) Public surfaces (`/` landing + `/how-it-works`): server-rendered, auth-aware via cookie inspection, content sourced from the `landing` and `howItWorks` admin-config namespaces with typed fallbacks in `config/content.ts`. Quick-link sets vary by auth state. The how-it-works page is tabbed (`?tab=` deep-linkable), with FAQs (Collapse) and an in-page playground registry (`modules/how-it-works/playgrounds.tsx`) — pure client demos with no DB writes. AppFooter renders a config-driven public-link strip (`content.footer.links`).
56) `<HelpLink />`: floating "Need help?" anchor mounted in the dashboard layout. Resolves the current pathname against `content.helpAnchors` (with first-segment fallback) and deep-links into `/how-it-works?tab=...`. Pages with no mapped anchor get a generic link to the index.
57) Mutation-auth audit: `npm run check:mutation-auth` (script `scripts/check-mutation-auth.ts`) statically scans every `app/api/**/route.ts` and fails when a `POST/PUT/PATCH/DELETE` export is missing a `verifyAuth(` call. Public auth lifecycle endpoints opt out via a file-level `// @public-mutation` annotation. The audit prevents future routes from silently bypassing the impersonation read-only chokepoint.
58) Impersonation campus-scope gate: `ImpersonationStartDialog` requires a target user when the impersonated role is in `CAMPUS_SCOPED_ROLES`/`GROUP_SCOPED_ROLES`. Without a target user, `auth.user.campusId` falls back to the SUPERADMIN's profile (typically null), and `materialiseAssignmentsForUser` short-circuits at `if (!campusId || !orgGroupId) continue` — silently. The dialog gate plus the QuickFormLandingPage banner make the gap visible and unblockable.
59) Audit-trail notes: every `createReportEvent` carries `details` populated by the workflow service (review reason, edit-request reason, edit reason). The bare events emitted directly from `app/api/reports/route.ts` (CREATED) and `app/api/reports/[id]/route.ts` (EDIT_APPLIED) now also stamp a brief change-summary so the timeline UI never shows a tag with no context. `ReportDetailPage` already renders `ev.details` per row when present.
60) Goal automation substrate: `lib/data/goalAutomation.ts` exposes `computeAutomatedGoal(priorAchieved, periodKind, config)` returning `{ value, appliedGrowth, tooltip, fromAutomation }`. Defaults: 5% w/w, 8% m/m, 15% y/y. Per-metric overrides keyed by `templateMetricId`. Live config behind `goalAutomation` admin-config namespace; client form pulls via `GET /api/goals/automation`. Tooltip surfaces source ("Auto-prefilled from prior week-on-week value of 245 (Week 17, 2026), with +5% growth applied. You can override.") so admins always know where a number came from.
61) `/how-it-works` Suspense boundary: the page wraps `<HowItWorksPage />` (which uses `useSearchParams` for `?tab=` deep-linking) in `<Suspense fallback={<LoadingSkeleton />}>` so prerender doesn't bail out. Required by Next 16's static-export contract — every client component using `useSearchParams` inside a server route must live inside a Suspense boundary.
62) Replay session affordance: `ImpersonationLogPanel` modal now exposes a Tabs split — Timeline (raw event log) and Replay narrative (humanised per-step reconstruction). A "Restart preview" CTA opens a fresh impersonation session with the same role + target user + mode, letting the SUPERADMIN re-walk a captured session. No DB writes — the narrative is constructed client-side from the existing event payload.
63) Mutation-auth audit: `npm run check:mutation-auth` (`scripts/check-mutation-auth.ts`) statically scans every `app/api/**/route.ts` and fails when a `POST/PUT/PATCH/DELETE` export lacks a `verifyAuth(` call. Public auth lifecycle endpoints opt out via the file-level `// @public-mutation` annotation. Wired into the npm scripts as `check:mutation-auth`.
64) Polymorphic org units: planned (see `temp-org-unit-polymorphism-plan-2026-05-04.md`). Today the substrate stores Campus + OrgGroup as concrete tables; the deep refactor that turns them into a single `OrgUnit` with admin-defined `level` + `parentId` + variable depth is deferred pending explicit approval — it touches every FK that points to either table and is multi-day work.
65) Polymorphic OrgUnit substrate (delivered 2026-05-05): `OrgUnit` is a single self-FK table carrying `level` (admin-defined string), `parentId`, and `rootKey` (groups roots into trees so multiple parallel hierarchies coexist). Migration `20260505120000_org_role_polymorphism_and_imports_xlsx` creates the table, mirrors every existing `org_groups` row (level=GROUP, rootKey=primary) and `campuses` row (level=CAMPUS, rootKey=primary) with the SAME UUID, and adds nullable `unitId` columns alongside every legacy carrier (`users`, `reports`, `invite_links`, `form_assignment_rules`). All migration steps use `IF NOT EXISTS` + `ON CONFLICT DO NOTHING`; existing rows are never overwritten.
66) Runtime Role substrate: `Role` rows seeded from the `UserRole` enum (with `isSystem=true`) coexist with the existing `User.role` enum column. Admins can CREATE custom roles via the new `RolesEditorV2`; SUPERADMIN's row is immutable; non-SUPERADMIN built-ins can be edited (label/cadence/capabilities) but only archived (soft-delete), never hard-deleted. SUPERADMIN-only capability bits (`canManageAdminConfig`, `canManageUsers`, `canManageOrg`, `canLockReports`) cannot be granted to non-SUPERADMIN roles via the create/update API.
67) `unitInScope(targetUnitId, scopeUnitIds)` (in `lib/data/orgUnitMatcher.ts`) is the canonical polymorphic scope check. Builds a graph cache (TTL 30s) of `(id, parentId)` rows once and runs BFS to test whether the target is at or beneath any scope id. `mergeLegacyScope` flattens legacy `campusId`/`orgGroupId`/`campusIds[]`/`orgGroupIds[]` fields into a single `unitIds[]` array so call sites can transition gradually.
68) Report visibility (`/api/reports/[id]` GET) now does a two-stage check: (1) the legacy `campusId === user.campusId` / `orgGroupId === user.orgGroupId` equality test (kept so legacy data resolves with no migration); (2) the polymorphic `unitInScope(report.unitId, [user.unitId])` fallback. Either path passing grants access — no regressions for legacy rows, and new multi-tree scopes work end-to-end.
69) Imports xlsx pipeline: `lib/data/importPipeline.ts → parseSpreadsheet({ format, payload, selectedSheet })` dispatches CSV → `parseCsv(text)` and XLSX → `parseXlsx(buffer)` (lazy SheetJS import, capped by `IMPORT_XLSX_MAX_SHEETS`). `app/api/imports/[id]/file` sniffs the upload (mime + filename), validates the workbook on upload, persists base64 in `storageRef` with `fileFormat=XLSX`, and returns `sheetNames[]` so the wizard can render a sheet picker. Failure paths return `{ code: "import_parse_failed" }` with status `400` instead of the prior 500.
70) `/api/admin-config/reconcile` (SUPERADMIN-only POST): runs `reconcileSubstrate({ dryRun })` which mirrors every existing OrgGroup + Campus row into OrgUnit (same UUIDs, never overwriting), seeds Role rows from `ROLE_CONFIG` (idempotent), and back-fills `User.{roleId,unitId}` + `Report.unitId` + `InviteLink.{targetRoleId,unitId}` + `FormAssignmentRule.{roleId,unitIds}` from the legacy columns. Re-runnable; never overwrites populated values. The Admin Config UI exposes both a dry-run preview (returns the would-be report without writing) and a live run.
71) Public-copy substrate: `loadPublicCopy(ns)` (in `lib/data/publicCopy.ts`) wraps `loadAdminConfig` with sanitisation (drops `<script>` tags, neutralises `javascript:`/`vbscript:` URLs, collapses protocol-relative `//host` prefixes, max field length) and deep-merge over the fallback so partial admin edits never blank out a section. The landing, how-it-works, about, privacy, terms, and footer pages all read through this loader.
72) Public-page rendering: `/`, `/how-it-works`, `/about`, `/privacy`, `/terms` are server-rendered with admin overrides via the `landing`, `howItWorks`, `aboutPage`, `privacyPage`, `termsPage` namespaces respectively. Hard-coded fallbacks live in `config/content.ts` (refreshed 2026-05-05 to reflect current platform reality including auto-totals, correlation panel, week-on-week, polymorphic hierarchy, role CRUD, xlsx imports).
73) How-it-works playgrounds: registry now includes seven additional demos — `analytics-chart-toggle`, `template-builder-effect`, `correlation-matrix-builder`, `auto-sum-chain`, `insight-summary-preview`, `aggregation-rollup`, `import-wizard-demo`. Each is a pure client component reusing the production algorithm; no DB writes, no extra API calls. Tab assignments in the `howItWorks` fallback are updated.
74) Admin Config: AdminConfigPage namespaces extended with `landing`, `howItWorks`, `aboutPage`, `privacyPage`, `termsPage`. The Roles tab embeds the legacy enum-overlay editor PLUS a new `RolesEditorV2` for the runtime Role table. The Hierarchy tab embeds the legacy level-label editor PLUS a new `OrgUnitTreeEditor` (multi-root tree with add/edit/promote/archive per node) PLUS a `ReconcilePanel` that runs the dry/live back-fill action.
75) `parseSpreadsheet` `SpreadsheetParseError` carries a `reason` field. API routes that catch it surface `{ success: false, error, code: "import_parse_failed" }` with HTTP `400` so the wizard can show a friendly message; no more raw Prisma stack traces escape the import flow.
76) Multi-root trees: `OrgUnit.rootKey` defaults to `"primary"`. `promoteToRoot(id, rootKey)` (called from `/api/org/units/[id]/promote`) snaps a node and all its descendants to a new `rootKey`, creating a parallel tree alongside the existing one. The OrgUnit tree editor renders one tab per `rootKey` with a "Promote" action on every node.
77) Migration discipline (re-stated): `prisma/migrations/20260505120000_org_role_polymorphism_and_imports_xlsx` is strictly additive — `CREATE TABLE IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`, `INSERT … ON CONFLICT DO NOTHING`. No `DROP`, no `RENAME`, no `TRUNCATE`. Apply via `prisma migrate deploy` only; `migrate reset` is forbidden in any data-bearing environment.
78) Permissions resolver back-compat: `lib/auth/permissions.ts → resolveRoleConfigMap` continues to read the `roles` admin-config namespace overlay against `ROLE_CONFIG`. `lib/data/role.ts → resolveRoleByCode` provides a parallel read API that prefers the runtime Role row when present and falls back to `ROLE_CONFIG`; future capability checks for runtime-created roles flow through this helper.
```
...
| `IMPORT_XLSX_MAX_SHEETS`                   | Cap on the number of sheets parsed from an XLSX workbook                              | `.env*`  | `12`                                  |
| `IMPORT_ALLOWED_MIMES`                     | Allowlist of accepted upload mimes for `/api/imports/[id]/file` (comma-separated)     | `.env*`  | `text/csv,application/...spreadsheetml.sheet,application/vnd.ms-excel` |
| `PUBLIC_COPY_DB_ENABLED`                   | Toggle DB-backed public-copy substrate. False = always use `config/content.ts`        | `.env*`  | `true`                                |
